...
var (
	buf = []byte{0xa, 0x8, 0x50, 0x72, 0x6f, 0x74, 0x6f, 0x62, 0x75, 0x66}
)

func Test_ProtocolProtobuf_Unmarshal(t *testing.T) {
	it := assert.New(t)
	pb := New()
...
func Test_ProtocolProtobuf_Marshal(t *testing.T) {
	it := assert.New(t)
	pb := New()
